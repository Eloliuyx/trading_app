...
def infer_amount_from_cv(df: pd.DataFrame) -> pd.Series:
    ...
    q95_vol = np.nanpercentile(df["Volume"].to_numpy(dtype=float), 95) if df["Volume"].notna().any() else 0.0
...
def main(cutoff: Optional[str]) -> None:
    ...
    for _, r in base.iterrows():
        ...
        rows.append({
            ...
            "features": {
                ...
                "f_exclude_st": (("ST" not in str(r["name"]).upper()) and ("ST" not in sym.upper())),
            },
        })
    ...
    dfu = pd.DataFrame([{"symbol": it["symbol"], "industry": it["industry"], **it["features"]} for it in rows])
